...
(function () {
  ...
  function buildRack(fp, tier) {
    ...
    // Safety-Bar quer, auf Steckbolzen in den Pfosten gelagert (nicht freischwebend)
    [-1, 1].forEach(function (sx) {
      cyl(g, r * 0.5, r * 1.1, c.iron, sx * (ux - r * 0.9), 0.42, -0.08, "x", 8); // Steckbolzen im Pfosten
      box(g, 0.05, 0.03, 0.09, c.dark, sx * (ux - r * 1.6), 0.42, -0.08);        // Auflage-Bracket
    });
  ...
  function buildChestPress(fp, tier) {
    ...
    box(g, 0.40, prem ? 0.66 : 0.58, 0.10, c.pad, 0, prem ? 0.9 : 0.82, -0.05, -0.2);
  ...
  /* ---------- optionale echte GLB-Modelle (in Blender gebaut, siehe
     assets/models/README.md) — ein Gerät pro Datei, ersetzen bei Erfolg
  ...
  function loadGLB(url, cb) {
    ...
    new THREE.GLTFLoader().load(url, function (gltf) {
      ...
      if (!root) { c.failed = true; finishGLB(url, null); return; }
  ...
  // zentriert + skaliert ein geladenes Modell auf die Gerätegrundfläche,
  // Boden auf y=0 — passend zur Item-Platzierung in rebuildRoom().
  function fitGlb(obj, fp, yawDeg) {
    ...
    obj.position.set(-ctr.x, -box3.min.y, -ctr.z);
  ...
  function rebuildRoom() {
    ...
    s.items.forEach(function (it) {
      ...
      if (glbCfg && typeof THREE.GLTFLoader !== "undefined" && !(glbCache[glbCfg.url] && glbCache[glbCfg.url].failed)) {
        ...
        (function (holder, cfg, fpp) {
          loadGLB(cfg.url, function (obj) {
            if (!obj || holder.parent !== roomGroup) return;
            while (holder.children.length) holder.remove(holder.children[0]);
            holder.add(fitGlb(obj, fpp, cfg.yaw || 0));
          });
        })(g, glbCfg, fp);
      }
